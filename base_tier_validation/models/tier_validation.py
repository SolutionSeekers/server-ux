...
class TierValidation(models.AbstractModel):
    ...
    def _add_comment(self, validate_reject):
        ...
        for item in definition_ids:
            for objeto in self.review_ids: 
                for cosa in objeto.reviewer_ids:
                    if cosa.id == self.env.user.id and objeto.definition_id == item:
                        id_def = item.id
                    else:
                        continue
        definition_ids_2 = self.env["tier.definition"].search(
            [
                ("model", "=", self._name),
                ("id","=",id_def)
            ]
        ...
        return {
            ...
            "context": {
                ...
                "default_definition_ids": definition_ids_2.ids,
    ...
    def _rejected_tier(self, tiers=False):
        ...
        for objeto in user_reviews: 
            for cosa in objeto.reviewer_ids:
                if cosa.id == self.env.user.id:
                    id_def = objeto.id
                else:
                    continue
        user_reviews_2 = self.env["tier.review"].search(
            [
                ("model", "=", self._name),
                ("id","=",id_def)
            ]
        )
        user_reviews_2.write(
            {
                "status": "rejected",
                "done_by": self.env.user.id,
                "reviewed_date": fields.Datetime.now(),
